...
import { selectChannelId, selectChannelName, selectDepId, selectDepName, selectStudentYear, selectYearId, selectYearName } from '../features/appSlice';
...
function Chat() {
    ...
    useEffect(() => {
        ...
        else if (channelId && departmentId && studyear) {
            db.collection("departments").doc(departmentId).collection("years").doc(studyear).collection("channels").doc(channelId).collection('messages').orderBy('timestamp','asc')
            .onSnapshot((snapshot) => 
                setMessages(snapshot.docs.map((doc) => doc.data()))
            ); 
        }
    ...
    const sendMessage = e => {
        ...
        if(studyear == null){
            ...
            db.collection("departments").doc(departmentId).collection("years").doc(studyear).collection('channels').doc(channelId).collection('messages').add({
                timestamp: firebase.firestore.FieldValue.serverTimestamp(),
                message: input,
                user: user,
            });
