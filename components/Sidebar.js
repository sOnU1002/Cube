...
import { selectDepId, selectDepName, selectStudentYear, selectYearId, selectYearName } from '../features/appSlice';
...
function Sidebar() {
  ...
  useEffect(() => {
    ...
    else if( studyear !=null ) {
      db.collection('departments').doc(departmentId).collection('years').doc(studyear)
      .collection('channels')
      .onSnapshot(snapshot => {
      setChannels(snapshot.docs.map(doc => ({
        id: doc.id,
